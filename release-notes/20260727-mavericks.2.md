...
## What's new in mavericks.2

- **Migration hardening (found by dogfooding on a real 10.9 box):** when the `default` → `container-tools`
  migration restarts the VM, VMware may hand it a new DHCP IP, which invalidates the VM's TLS certificate.
  Container Tools now detects that specific cert/IP mismatch and **regenerates the certificate
  automatically**, so the `mavericks` docker context comes up without any manual `regenerate-certs` step.
  Gated tightly (only that error) so it never restarts the daemon unnecessarily.

## Migration from Wowfunhappy's "Docker for Mavericks"

Done automatically the first time Container Tools reconciles:
...
<https://github.com/Mavergreen/container-tools> for the one-line `docker-machine create` setup.
